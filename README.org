...
\]
where $x \in [1, N - 1]$ is the interaction diagram label, $\omega_{\mathbf{p}}$ is the energy of the phonon mode *p*, and $S$ is sign metric defined as $S(x, i) = +$ for $x \leq i$ and $-$ otherwise.
